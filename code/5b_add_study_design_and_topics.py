...
Output: data/analysis/analysis_dataset_enriched_v2b.csv
...
OUTPUT_CSV = ROOT / "data" / "analysis" / "analysis_dataset_enriched_v2b.csv"
...
def _assign_design_from_text(text: str) -> str:
    ...
    if (
        "time series" in text
        and "interrupted time series" not in text
        and "natural experiment" in text
    ):
        return "Ecological / Time-series"
...
def _all_designs_matching_keywords(keywords: list[str]) -> list[str]:
    ...
    return [
        ...
        for group in DESIGN_HIERARCHY
        if any(
            _keyword_pattern_matches(pat, tok)
            for tok in keywords
...
def _keyword_pattern_matches(pat: str, tok: str) -> bool:
    """Match keyword patterns, handling short acronyms conservatively."""
    if pat == "rct":
        return re.search(r"\brcts?\b", tok) is not None
    return pat in tok
...
def assign_design_keywords(row: pd.Series) -> str:
    ...
    for group in DESIGN_HIERARCHY:
        ...
        if any(_keyword_pattern_matches(pat, tok) for tok in keywords for pat in patterns):
            return group
    return "Other/None"
